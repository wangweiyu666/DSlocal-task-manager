...
test("creates a draft task and reaches DST1 preview", async ({ page }, testInfo) => {
  ...
  await expect(page.getByLabel(/^归属日期（留空默认今天/)).toBeVisible();
  await expect(page.getByLabel(/^归属日期（留空默认今天/)).toHaveAttribute("lang", "en-CA");
...
test("separates recurring tasks at creation", async ({ page }) => {
  ...
  await expect(page.getByLabel(/^开始日期（留空默认今天/)).toBeVisible();
  await expect(page.getByLabel(/^开始日期（留空默认今天/)).toHaveAttribute("lang", "en-CA");
  await expect(page.getByLabel(/^归属日期/)).toHaveCount(0);
