...
export default function ConnectedApp() {
  ...
  return <div className="connected-shell" data-build-channel={__BUILD_CHANNEL__}>
    <aside className="connected-rail"><div><p className="eyebrow">联网版 · 管理员</p><h2>{membership?.space.name}</h2><span>{membership?.space.timeZone}</span></div>
      <nav><button className={tab === "tasks" ? "active" : ""} onClick={() => setTab("tasks")}><ListTodo />任务库</button><button className={tab === "groups" ? "active" : ""} onClick={() => setTab("groups")}><FolderKanban />积分组</button><button className={tab === "results" ? "active" : ""} onClick={() => setTab("results")}><Send />结果</button><button className={tab === "notifications" ? "active" : ""} onClick={() => void loadNotifications()}><Bell />通知</button><button className={tab === "audit" ? "active" : ""} onClick={() => void loadAudit()}><ShieldAlert />审计</button><button className={tab === "settings" ? "active" : ""} onClick={() => setTab("settings")}><Settings />设置</button></nav>
