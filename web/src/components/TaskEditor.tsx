...
export function TaskEditor({ value, groups, onChange, allowKindChange = false, advancedContent }: TaskEditorProps) {
  ...
  return <div className="task-editor">
    ...
    <section className="form-section"><h3>完成方式</h3><div className="form-grid">
      <label className="field"><span>执行类型</span><select value={value.execution?.k ?? 0} onChange={(event) => { const kind = Number(event.target.value); patch({ name: kind === 4 && !value.name.trim() ? "今天的心情怎么样" : value.name, execution: kind === 1 ? { k: 1, a: 2, v: 10 } : kind === 2 ? { k: 2, v: 600 } : kind === 3 ? { k: 3 } : kind === 4 ? { k: 4 } : null }); }}><option value={0}>直接完成</option><option value={1}>完成指定次数</option><option value={2}>完成指定时长</option><option value={3}>填写信息告知</option><option value={4}>心情记录</option></select></label>
