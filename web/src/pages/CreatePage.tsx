...
import { ArrowLeft, Clipboard, Copy, FilePlus2, Plus, Send, Trash2 } from "lucide-react";
...
export function CreatePage() {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "n") { event.preventDefault(); addTask(null); }
    };
    ...
    <div className={`create-layout ${selectedTask ? "has-selection" : ""}`}>
      <section className="batch-panel">
        ...
        <div className="batch-tree">
          {treeGroups.map((group) => <section className="tree-group" key={group.id ?? "ungrouped"}><div className="tree-group-heading"><div><strong>{group.name}</strong><span>{groupedTasks.get(group.id)?.length ?? 0} 项</span></div><div>{group.id && <label className="small-check"><input type="checkbox" checked={draft.includeGroupIds.includes(group.id)} onChange={(event) => writeDraft({ includeGroupIds: event.target.checked ? [...draft.includeGroupIds, group.id!] : draft.includeGroupIds.filter((id) => id !== group.id) })} />发送组资料</label>}<button className="icon-button" onClick={() => addTask(group.id)} aria-label={`在${group.name}添加任务`}><Plus size={19} /></button></div></div>
            <div className="tree-tasks">{(groupedTasks.get(group.id) ?? []).map((task) => <button key={task.draftItemId} className={`tree-task ${selectedItemId === task.draftItemId ? "selected" : ""}`} onClick={() => setSelectedItemId(task.draftItemId)}><span>{task.name || "未命名任务"}</span><small>{task.required ? "必做" : "选做"} · {task.points} 分 · {task.source === "existing" ? "更新" : task.source === "delay" ? "延期" : "新建"}</small></button>)}</div>
          </section>)}
        </div>
        ...
        <div className="batch-summary"><span>{draft.tasks.length} 个任务</span><span>{draft.includeGroupIds.length} 个组资料更新</span><span>{draft.cancellations.length} 个撤销</span></div>
      </section>
      <section className="editor-panel">
        {selectedTask ? <><div className="editor-toolbar"><button className="button text mobile-only" onClick={() => setSelectedItemId(null)}><ArrowLeft size={18} />返回批次</button><div><strong>{selectedTask.name || "编辑任务"}</strong><small className="mono">{selectedTask.taskId}</small></div><button className="button text danger" onClick={removeSelectedTask}><Trash2 size={17} />移除</button></div><TaskEditor value={selectedTask} groups={groups} onChange={changeSelectedTask} /></> : <div className="empty-state"><Clipboard size={44} /><h2>选择任务开始编辑</h2><p>可在任意积分组标题旁点击“＋”，或从模板添加任务。</p><button className="button primary" onClick={() => addTask(null)}><Plus size={18} />添加未分组任务</button></div>}
      </section>
    </div>
