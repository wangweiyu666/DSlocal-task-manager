...
import { Ban, CalendarClock, Copy, FileInput, History, Plus, RotateCcw, Search } from "lucide-react";
...
export function LibraryPage() {
  ...
  const addToDraft = async (task: TaskRecord, source: "existing" | "delay" = "existing") => {
    ...
    show(source === "delay" ? "已加入当前草稿；请修改截止时间后生成" : "已加入当前草稿", "success"); navigate("/create");
  };
  const cancelInDraft = async (task: TaskRecord) => {
    ...
    await updateDraft(draft.id, { cancellations: [...new Set([...draft.cancellations, task.id])] }); show("撤销操作已加入当前草稿", "success"); navigate("/create");
  };
  ...
  return <div className="page">
    <header className="page-header"><div><p className="eyebrow">稳定 taskId · 完整新版</p><h1>任务库</h1><p>管理 Dom 已创建的任务定义；这里不显示 Sub 的执行状态。</p></div><button className="button tonal" onClick={() => setRestoreText("")}><FileInput size={18} />粘贴旧字符串</button></header>
    ...
    {filtered.length === 0 ? <div className="empty-state"><Search size={44} /><h2>{tasks.length ? "没有匹配的任务" : "任务库还是空的"}</h2><p>在创建页生成第一个批次，或从旧 DST1 字符串恢复。</p></div> : <div className="card-list">{filtered.map((task) => <article className="data-card" key={task.id}><div className="data-card-main" onClick={() => setEditing(task)}><div className="card-title-row"><h2>{task.name}</h2><span className={`status-pill ${task.required ? "required" : "optional"}`}>{task.required ? "必做" : "选做"}</span></div><p>{task.description || "无描述"}</p><div className="meta-row"><span>{groupMap.get(task.groupId ?? "") ?? "未分组"}</span><span>{task.points} 分</span><span>v{task.version}</span><span className="mono">{task.id}</span></div></div><div className="card-actions"><button className="button tonal" onClick={() => addToDraft(task)}><Plus size={16} />生成新版</button><button className="button text" onClick={() => duplicateTask(task)}><Copy size={16} />复制</button><button className="button text" onClick={() => addToDraft(task, "delay")}><CalendarClock size={16} />延期</button><button className="button text" onClick={() => addToDraft(task)}><RotateCcw size={16} />恢复</button><button className="button text danger" onClick={() => cancelInDraft(task)}><Ban size={16} />撤销</button></div>{revisions.some((item) => item.taskId === task.id) && <div className="revision-note"><History size={15} />已保存 {revisions.filter((item) => item.taskId === task.id).length} 个不可变版本</div>}</article>)}</div>}
