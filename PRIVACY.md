...
DStationery 不申请网络权限，不收集、上传或出售任何数据。任务、备注、积分、历史和设置仅保存在 Android 应用沙箱。

DSTB1 是用户主动导出的未加密备份，可能包含完整任务和历史。分享图片、备份与诊断文件只在用户主动操作时创建。诊断文件排除任务内容、业务 ID、文件路径、设备标识和异常堆栈。
