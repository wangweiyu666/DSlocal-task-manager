...
if ($evidence.releaseKind -ne 'executor-production') { throw 'Evidence is not for the production executor app.' }
if ($evidence.status -ne 'approved') { throw 'Release evidence status must be approved.' }
if ($evidence.packageName -ne 'com.ds.localtaskmanager.connected.production') { throw 'Unexpected executor package name.' }
if ($evidence.cloudEnvironment -ne 'production') { throw 'Executor release must target production.' }
...
$apk = Join-Path (Split-Path -Parent $EvidencePath) "DStationery-executor-$($evidence.version).apk"
...
$tag = "executor-v$($evidence.version)"
if (git tag --list $tag) { throw "Tag already exists: $tag" }
$confirmation = if ($ConfirmTag) { $ConfirmTag } else { Read-Host "Type $tag to create the public executor prerelease" }
if ($confirmation -ne $tag) { throw 'Publishing cancelled.' }
git tag -a $tag -m "DStationery executor $($evidence.version)"
...
gh release create $tag $apk "$apk.sha256" (Join-Path (Split-Path -Parent $EvidencePath) 'THIRD_PARTY_NOTICES.txt') $EvidencePath `
    --repo $Repository --title "DStationery Executor $($evidence.version)" --prerelease --generate-notes
if ($LASTEXITCODE -ne 0) { throw 'Unable to create the public GitHub prerelease.' }
